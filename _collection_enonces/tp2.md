...
* Créer ces 2 classes (`Vehicule` et `Velo`) puis une troisième classe pour faire des tests `VehiculesTests` dans laquelle il faudra ajouter une méthode `main` pour créer un Vehicule et un Velo et vérifier le comportement du programme lorsqu'on appelle chacune des méthodes.

* Vérifier qu'il est bien possible d'appeler la méthode `getVitesse` qui n'a pas fait l'objet d'une redéfinition sur la sous-classe `Velo` afin de mettre en évidence le fait que les sous-classes héritent des méthodes définies sur leur super-classe.

* Vérifier en créant deux objets de type `Velo` distincts qu'ils peuvent bien avoir chacun une vitesse déifférente afin de mettre en évidence le fait que les sous-classes héritent des variables d'instance de leur super-classe.

* Compléter la hiérarchie d'héritage avec les classes suivantes en prenant soin de remplacer les `...` par le code approprié
    ...
    public class Train extends Vehicule {
        ...
        public void faireDuBruit() {
            super.faireDuBruit();
            System.out.println("Tchou tchou le p'tit train");
        }
...
Le polymorphisme consiste à écrire du code qui ne dépend pas du type concret des objets. Utilisation de variables ou de paramètres de fonctions de type `Vehicule` mais à qui on pourra passer indifféremment des instances de n'importe laquelle des classes de la hiérarchie. Exemple :
...
* Modifier le code de la classe `VehiculesTests`en créant un variable de type `Vehicule` qui référencera successivement un vélo, une voiture, une voiture automatique,...
    ...
    - Vérifier qu'il n'est possible d'appeler que les méthodes disponibles sur la classe `Vehicule` (impossible d'utiliser `getRapportVitesse()` même si l'obet effectivement référencé est de type `VoitureAutomatique`
